...
/** Local, credential-free toggle state written by the Plugins settings page. */
const PLUGIN_TOGGLE_FILENAME = 'plugin-toggles.json'

/** Read the persisted plugin preferences as Loader patches for this boot. */
function loadPluginTogglePatches(): PatchOptions[] {
  const path = join(resolveDshHome(), PLUGIN_TOGGLE_FILENAME)
  if (!existsSync(path)) return []
  try {
    const parsed: unknown = JSON.parse(readFileSync(path, 'utf8'))
    if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) return []
    return Object.entries(parsed)
    // `include` is the Loader's root entry. Persisting it disabled makes the
    // whole plugin tree unavailable on the next launch, so never honor it.
      .filter(([entryId]) => entryId !== 'include')
      .filter((entry): entry is [string, boolean] => typeof entry[1] === 'boolean')
      .map(([id, enabled]) => ({ id, disabled: !enabled }))
  } catch {
    // A damaged convenience preference must never prevent the desktop app
    // from starting. It contains only plugin IDs and booleans, never secrets.
    process.stderr.write(`${NAME}: ignoring unreadable ${PLUGIN_TOGGLE_FILENAME}\n`)
    return []
  }
}
...
interface ComposedProfile {
  ...
  /** Credential-free plugin enablement preferences from the desktop settings page. */
  pluginTogglePatches: PatchOptions[]
...
function composeProfile(
  ...
  const pluginTogglePatches = loadPluginTogglePatches()
  const overlays = patchFiles.flatMap(file => loadOverlayPatches(NAME, resolve(file)))
  const bundlePatches = profile.layers.flatMap(layer => layer.patches)
  const rows = new Map<string, EntryOptions>()
  for (const row of composeEntries([bundlePatches, profile.patches, homePatches, pluginTogglePatches, overlays])) {
  ...
  return { profile, bundlePatches, homePatches, pluginTogglePatches, overlays: composedOverlays, rows }
}
