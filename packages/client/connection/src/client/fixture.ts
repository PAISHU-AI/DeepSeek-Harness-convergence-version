  ...
  const api: ApiProxy = {
    sessions: {
      ...
      prompt: (request) => {
        ...
        summary.blank = false
        const userText = content.map(b => (b.type === 'text' ? b.text : b.type === 'document' ? b.text : '')).join('')
        const durable: ContentBlock[] = content.map((block) => {
          if (block.type === 'text') return block
          if (block.type === 'document') {
            return {
              type: 'text',
              text: `[Attachment: ${block.name}]\n${block.text}${block.truncated ? '\n[content truncated]' : ''}\n[/Attachment]`,
              document: { name: block.name, format: block.format, truncated: block.truncated },
            }
          }
    ...
    workspace: {
      ...
      readFile: (request) => {
        ...
        if (path.includes('..') || path.startsWith('/') || path === '') {
          return err(request, { code: 'workspace-file-invalid', message: `invalid artifact path ${path}`, details: { workspaceId, path } })
        }
        return ok(request, { kind: 'text' as const, content: `# ${path}\n\nFixture artifact preview.` })
      },
      openFile: (request) => {
        const { workspaceId, path } = request.payload
        if (!workspaces.some(workspace => workspace.workspaceId === workspaceId)) {
      ...
      writeFile: (request) => {
        ...
        const bytes = 'content' in request.payload
          ? new TextEncoder().encode(request.payload.content).byteLength
          : Uint8Array.from(atob(request.payload.data), char => char.charCodeAt(0)).byteLength
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'workspace.readFile': return this.api.workspace.readFile(request)
      case 'workspace.openFile': return this.api.workspace.openFile(request, signal)
      case 'workspace.writeFile': return this.api.workspace.writeFile(request)
