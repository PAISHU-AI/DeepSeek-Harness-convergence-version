...
/** Product copy for the built-in slash-command catalog. Command tokens stay
 * machine-stable; their explanatory text follows the Chinese desktop shell. */
const COMMAND_DESCRIPTIONS_ZH: Readonly<Record<string, string>> = {
  compact: '压缩较早的对话历史',
  export: '将本次会话记录下载为 ZIP 压缩包',
  feedback: '提交本次会话的反馈',
  goal: '设置或查看长期任务目标',
  model: '选择本次会话使用的模型',
  permission: '切换访问权限模式',
...
function localizedDescription(name: string, description: string): string {
  return COMMAND_DESCRIPTIONS_ZH[name] ?? description
}
...
export class CommandUiRuntime extends Service implements CommandUiContract {
  ...
  private async candidates(session: ClientSessionContext, req: CandidateRequest): Promise<readonly InputTriggerCandidate[]> {
    ...
    for (const c of list) {
      ...
      rows.push({
        name: c.name,
        description: localizedDescription(c.name, c.description),
