...
import { UploadFileError } from './attachments/file-attachments.ts'
...
/**
 * Convert only a path rooted in the current workspace into the strict
 * slash-separated relative spelling required by the artifact Host API.
 * Paths outside that root deliberately remain unavailable to in-app preview.
 */
function workspaceArtifactPath(cwd: string | undefined, path: string): string | undefined {
...
export function apply(ctx: Context): void {
    ...
    inject: (sessionId: SessionId | undefined): ComposerBarInjected => {
      ...
      return {
        ...
        addAttachments: async (files) => {
          try {
            const attachments = await conversation.createDraftAttachments(files)
            ...
            if (error instanceof UploadFileError) {
              return error.reason === 'unsupported-type' || error.reason === 'mismatched-type'
                ? t('upload.unsupportedType')
                : error.reason === 'empty-document'
                  ? t('upload.emptyDocument')
  ...
  slots.register({
    ...
    inject: (sessionId: SessionId, actions: BoundActions<typeof chatStore>): ChatViewInjected => {
      ...
      return {
        openDetails: (target) => {
          actions.setArtifactPreview(null)
        ...
        previewFile: (path) => {
          const cwd = sessions.list.getSnapshot().byId[sessionId]?.cwd
          const workspace = workspaces.list.getSnapshot().items.find(item => item.path === cwd)
          const artifactPath = workspaceArtifactPath(cwd, path)
          // An unregistered session root has no Host workspace authority for
          // preview/editing. So do paths outside that registered root; keep
          // the previous native-open behavior rather than broadening access.
          if (workspace === undefined || artifactPath === undefined) {
            void workspaces.openPath(resolveWorkspacePath(cwd, path)).catch(() => {})
            return
          }
          actions.select(null)
          actions.setArtifactPreview({ kind: 'workspace', workspaceId: workspace.workspaceId, path: artifactPath })
          layout.openDetails()
        },
        previewDocument: (target) => {
          actions.select(null)
          actions.setArtifactPreview(target)
          layout.openDetails()
        },
