...
import type { DocumentPreviewTarget } from '../contract/views.ts'
...
function contentParts(content: readonly unknown[]): {
  ...
  documents: DocumentPreviewTarget[]
  rest: unknown[]
} {
  const texts: string[] = []
  const images: { attachment: UserImage['attachment'] }[] = []
  const documents: DocumentPreviewTarget[] = []
  const rest: unknown[] = []
  for (const block of content) {
    const b = block as { type?: string; text?: string; attachment?: unknown }
    if (b.type === 'text' && typeof b.text === 'string' && isDocumentBlock(b)) {
      documents.push({ kind: 'document', ...b.document, text: b.text })
    }
...
function isDocumentBlock(value: { type?: string; text?: string; document?: unknown }): value is { type: 'text'; text: string; document: Omit<DocumentPreviewTarget, 'kind' | 'text'> } {
  const document = value.document as Partial<DocumentPreviewTarget> | undefined
  return document?.kind === undefined && typeof document?.name === 'string'
    && (document?.format === 'pdf' || document?.format === 'docx' || document?.format === 'text' || document?.format === 'markdown' || document?.format === 'csv' || document?.format === 'json')
...
function UserStyleBubble({
  content, imageLoader, previewDocument, actions, pending = false, t,
}: {
    ...
    <div className={css.userRow} data-pending-steering={pending || undefined} data-time-hover-root>
      <div className={css.userStack}>
        ...
        {documents.map(document => (
          <button key={`${document.format}:${document.name}`} type="button" className={css.documentChip} onClick={() => { previewDocument(document) }}>
            {document.format.toUpperCase()} · {document.name}
          </button>
        ))}
...
export const UserMessageNodeView = memo(function UserMessageNodeView({
  node, loadImage, previewDocument, t,
