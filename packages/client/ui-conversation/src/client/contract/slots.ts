...
import type { ArtifactPreviewTarget, CallId, DocumentPreviewTarget, SelectionTarget, ViewTab } from './views.ts'
...
/** Browser-local document projection; original bytes are discarded after extraction. */
export interface ComposerDocumentAttachment {
  kind: 'document'
  id: DraftAttachmentId
  fileName: string
  format: 'pdf' | 'docx' | 'text' | 'markdown' | 'csv' | 'json'
  bytes: number
  text: string
  truncated: boolean
}

/** One temporary item in the composer attachment list. */
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /** The selected workspace artifact's preview/editor, hosted by DetailsPanel. */
...
export interface TurnTailOwnerProps {
  ...
  /** Select a generated artifact for in-app right-panel preview/editing. */
...
export interface ChatNodeOwnerProps {
  ...
  openFile: (path: string) => void
  previewFile: (path: string) => void
  previewDocument: (target: DocumentPreviewTarget) => void
...
/** Owner currency for the optional right-panel artifact preview/editor. */
...
export interface ComposerBarInjected {
  ...
  /** Asynchronously parse and append images or supported documents. */
  addAttachments: ((files: readonly File[]) => Promise<string | null>) | undefined
  /** Release one image or extracted document and remove its id from the input. */
  removeAttachment: ((id: DraftAttachmentId) => void) | undefined
  /** Resolve ordered ids to all browser-owned draft attachments. */
...
export interface ChatViewInjected {
   ...
   */
  openFile: (path: string) => void
  /** Select a generated artifact for the Details-panel preview/editor. */
  previewFile: (path: string) => void
  /** Open a parsed uploaded document in the shared right details panel. */
  previewDocument: (target: DocumentPreviewTarget) => void
  loadOlder: () => void
  /** Resolve a session-authorized historical image for inline display. */
  loadImage: (attachment: ImageAttachmentRef) => Promise<string>
...
export type DetailsSlotProps = PropsRuntime<'details'>
  & PropsRenderSlots<'conversation.details.tool' | 'conversation.details.artifact'>
