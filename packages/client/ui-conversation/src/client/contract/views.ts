...
/** One output file selected for the right-side preview/editor surface. */
export interface ArtifactPreviewTarget {
  kind: 'workspace'
  /** Registered workspace authorizing the Host file read/write. */
  workspaceId: WorkspaceId
  /** Safe workspace-relative path emitted by a successful mutation tool. */
  path: string
}

/** Parsed upload content held only in the durable transcript and shown on demand. */
...
export type DetailsPreviewTarget = ArtifactPreviewTarget | DocumentPreviewTarget
...
export interface ChatStoreState {
  ...
  /** Artifact selected from a completed turn; takes precedence in Details. */
  artifactPreview: DetailsPreviewTarget | null
