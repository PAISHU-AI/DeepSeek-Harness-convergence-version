...
import {
  assertDocumentBatch, classifyUpload, MAX_DOCUMENT_BATCH_CHARACTERS, parseDocument, UploadFileError,
} from './attachments/file-attachments.ts'
...
export class ConversationController extends Service implements IConversation {
  ...
  /**
   * Create an all-or-nothing batch of images or locally extracted documents.
   * @param files - browser-selected files in user-selected order.
   * @returns runtime-only attachment descriptors.
   */
  async createDraftAttachments(files: readonly File[]): Promise<readonly ComposerAttachment[]> {
    const classifications = files.map(file => classifyUpload(file))
    assertDocumentBatch(files)
    // Finish every fallible document read before allocating image object URLs:
    // an atomic rejected batch must not leave untracked preview resources.
    const parsed = await Promise.all(files.map(async (file, index) => {
      const classification = classifications[index]
      if (classification === undefined) throw new Error('Missing upload classification')
      return classification.kind === 'document' ? parseDocument(file) : undefined
    }))
    const attachments = files.map((file, index) => {
      const classification = classifications[index]
      if (classification === undefined) throw new Error('Missing upload classification')
      if (classification.kind === 'image') return browserDraftAttachment(file)
      const document = parsed[index]
      if (document === undefined) throw new Error('Missing parsed upload document')
      return {
    ...
    const documentCharacters = attachments.reduce((total, attachment) =>
      total + (attachment.kind === 'document' ? attachment.text.length : 0), 0)
    if (documentCharacters > MAX_DOCUMENT_BATCH_CHARACTERS) throw new UploadFileError('batch-text-too-large')
  ...
  /** Resolve ordered ids to every current draft attachment. */
  ...
  private async serializeAttachments(attachments: readonly ComposerAttachment[]): Promise<Parameters<SessionFace['prompt']>[0]> {
    return Promise.all(attachments.map(async (attachment) => {
      ...
      const [image] = await this.serializeImages([attachment.file])
      if (image === undefined) throw new Error('conversation.serializeAttachments: image serialization produced no part')
      return image
    }))
  }
}
