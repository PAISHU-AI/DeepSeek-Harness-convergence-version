...
function DocumentPreview({ artifact, t }: { artifact: DocumentPreviewTarget; t: DetailsPanelProps['t'] }) {
  return (
    <section className={css.documentPreview}>
      {artifact.truncated && <div className={css.documentNotice}>{t('json.truncated', { total: artifact.text.length })}</div>}
      {artifact.format === 'markdown'
        ? <MarkdownText text={artifact.text} />
        : <CodeBlock code={artifact.text} lang={artifact.format === 'text' ? 'text' : artifact.format} copyLabel={t('copy')} copiedLabel={t('copied')} />}
    </section>
  )
}
...
export function DetailsPanel({ useSession, useSessions, sessionId, useStore, renderSlot, closeDetails, t }: DetailsPanelProps) {
  ...
  const artifact = useStore(s => s.artifactPreview ?? null)
    ...
    <div className={css.root}>
      <div className={css.header}>
        <div className={css.title}>
          {artifact?.kind === 'workspace' ? artifact.path : artifact?.kind === 'document' ? artifact.name : (selection === null ? t('details.title') : material?.name ?? selection.toolName ?? t('details.title'))}
        </div>
        <button
      ...
      <div className={css.body}>
        {artifact?.kind === 'document'
          ? <DocumentPreview artifact={artifact} t={t} />
          : artifact?.kind === 'workspace'
            ? renderSlot('conversation.details.artifact', { artifact }, {
              fallback: <div className={css.empty}>Preview is unavailable in this workspace.</div>,
            })
