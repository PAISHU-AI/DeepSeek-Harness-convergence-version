...
import {
  IconChevronRightOutline14, IconPaperclipOutline16, IconPlusOutline16, IconWarningOutline16, RiskConfirmation, Toast, Tooltip,
...
import type { ComposerBarProps, ComposerImageAttachment } from '../contract/slots.ts'
import { UPLOAD_ACCEPT } from '../attachments/file-attachments.ts'
...
import { FULL_ACCESS, permissionOptionLabel, PermissionSubmenu } from './PermissionSelect.tsx'
import css from './InputBar.module.css'

/** Decoration product of the no-session state (no machine, empty draft). */
const INERT_DECORATIONS: DraftDecorations = { token: null, chips: [], textRefs: [], hint: null }

/** Compact command catalog for the hover menu; picking one inserts its stable slash token. */
const TOOL_COMMANDS = [
  { name: 'compact', description: '压缩较早的对话历史' },
  { name: 'export', description: '将本次会话记录下载为 ZIP 压缩包' },
  { name: 'feedback', description: '提交本次会话的反馈' },
  { name: 'goal', description: '设置或查看长期任务目标' },
  { name: 'permission', description: '切换访问权限模式' },
...
export function InputBar({
  ...
  addAttachments, removeAttachment, draftAttachments,
  ...
  const attachments = useMemo(() => {
    ...
    return draftAttachments?.(input.imageIds) ?? draftImages?.(input.imageIds) ?? []
  }, [draftAttachments, draftImages, input?.imageIds])
  const empty = draft.trim() === '' && attachments.length === 0
  const [preview, setPreview] = useState<ComposerImageAttachment | null>(null)
  const [dragActive, setDragActive] = useState(false)
  const [intakingFiles, setIntakingFiles] = useState(false)
  const [toolsOpen, setToolsOpen] = useState(false)
  const [toolsSubmenu, setToolsSubmenu] = useState<'mode' | 'command' | null>(null)
  ...
  const selectedPermissionLabel = useMemo(() => {
    ...
    return current === undefined ? permissions.currentValue : permissionOptionLabel(current, t)
  }, [permissions, t])
  ...
  useEffect(() => {
    if (locked) {
      setToolsOpen(false)
      setToolsSubmenu(null)
      setFullAccessConfirmationOpen(false)
      setFullAccessAcknowledged(false)
    }
  }, [locked])
  // Hover selects a secondary panel, while a pointer press anywhere outside
  // this one menu is an explicit dismissal. Keeping this on `pointerdown`
  // makes clicks in the editor/right preview close before their own action,
  // and the containment check keeps menu/submenu interactions intact.
  useEffect(() => {
    if (!toolsOpen) return
    const dismissOutside = (event: PointerEvent): void => {
      const target = event.target
      // The Full access acknowledgement is portaled to document.body. It is
      // logically part of this menu flow even though it is not a DOM child of
      // toolsMenuRef; dismissing here would unmount the submenu before the
      // checkbox or confirmation button receives its click.
      if (target instanceof Element && target.closest('[role="dialog"][aria-modal="true"]') !== null) return
  ...
  const machineBusy = input?.phase === 'adjudicating' || input?.phase === 'submitting' || intakingFiles
    ...
    const rejected = ((): string | null => {
      if (imageLimits !== undefined) {
        ...
        const total = attachments.reduce((sum, attachment) =>
          sum + (attachment.kind === 'image' ? attachment.file.size : 0), 0)
  ...
  const intakeFiles = useCallback(async (files: readonly File[]): Promise<void> => {
    ...
    setIntakingFiles(true)
    try {
      const rejected = await addAttachments(files)
      if (rejected !== null) showToast(rejected)
    } finally {
      setIntakingFiles(false)
    }
  }, [addAttachments, intakeImages, showToast])
  ...
  const canAcceptDrop = !locked && !machineBusy && (addAttachments !== undefined || addImages !== undefined)
  useEffect(() => {
  ...
  const onOpenCommands = (): void => {
    setToolsSubmenu('command')
  }

  const chooseToolCommand = (name: string): void => {
    if (inputActions === undefined || locked) return
  ...
  // The Access submenu is projection-fed. It only mounts while its parent
  // menu is hovered, so the composer never presents a second selector card.
  ...
  const confirmFullAccess = (): void => {
    if (locked || !fullAccessAcknowledged || command === undefined) return
    closeFullAccessConfirmation()
    void command(`/permission ${FULL_ACCESS}`).catch(() => false)
  }
      ...
      >
          ...
          <div className={css.attachments}>
            {railItems.length > 0 && (
              <AttachmentRail
                ...
                onRemove={(item) => { (removeAttachment ?? removeImage)?.(item.attachment.id) }}
              />
            )}
            {documents.map(document => (
              <div key={document.id} className={css.documentChip}>
                <span className={css.documentKind}>{document.format.toUpperCase()}</span>
                <span className={css.documentName} title={document.fileName}>{document.fileName}</span>
                <button
                  type="button"
                  className={css.documentRemove}
                  aria-label={t('upload.remove', { name: document.fileName })}
                  onClick={() => { (removeAttachment ?? removeImage)?.(document.id) }}
                >×</button>
              </div>
            ))}
          </div>
        ...
        <div className={css.row}>
          <div className={css.tools}>
            <div className={css.toolsPrimary}>
              <div ref={toolsMenuRef} className={css.toolsMenu}>
                <Tooltip label={t('input.tools')} side="top" delayMs={500}>
                  <button
                    type="button"
                    className={css.add}
                ...
                {toolsOpen && (
                  <div className={css.toolsPopover} role="menu" aria-label={t('input.tools')}>
                    <button
                      type="button"
                      className={css.toolsMenuItem}
                      role="menuitem"
                      aria-label={t('input.toolsMode')}
                      aria-haspopup="menu"
                      aria-expanded={toolsSubmenu === 'mode'}
                      onMouseEnter={() => { setToolsSubmenu('mode') }}
                      onFocus={() => { setToolsSubmenu('mode') }}
                      onClick={() => { setToolsSubmenu('mode') }}
                    >
                      <span>{t('input.toolsMode')}</span>
                    ...
                    <button
                      ...
                      aria-haspopup="menu"
                      onMouseDown={keepFocus}
                      onMouseEnter={onOpenCommands}
                      onFocus={onOpenCommands}
                      onClick={onOpenCommands}
                    >
                      <span>{t('input.commands')}</span>
                      <IconChevronRightOutline14 aria-hidden />
                    </button>
                    {toolsSubmenu === 'mode' && (
                      <div className={css.toolsSubmenu}>
                        {accessSelect}
                        {renderSlot('conversation.input.plan', { locked })}
                      </div>
                    )}
                    {toolsSubmenu === 'command' && (
                      <div className={`${css.toolsSubmenu} ${css.commandSubmenu}`} role="menu" aria-label={t('input.commands')}>
                        {TOOL_COMMANDS.map(item => (
                          <button
                            key={item.name}
                            type="button"
                            role="menuitem"
                            className={css.commandMenuItem}
                            onMouseDown={keepFocus}
                            onClick={() => { chooseToolCommand(item.name) }}
                          >
                            <span>{item.name}</span>
              ...
              {selectedPermissionLabel !== null && !locked && (
                <span
                  className={css.toolsPermission}
                  role="status"
                  aria-label={t('input.accessMode', { name: selectedPermissionLabel })}
                >
            ...
            <button
              ...
              disabled={locked || machineBusy || (addAttachments === undefined && addImages === undefined)}
              onMouseDown={keepFocus}
