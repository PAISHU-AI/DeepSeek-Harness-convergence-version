...
/** Shared Chinese/locale-aware label for the current chip and every permission menu surface. */
export function permissionOptionLabel(option: PermissionSelectValue['options'][number], t: PermissionSelectProps['t']): string {
  if (option.value === 'read-only') return t('input.permission.readOnly')
  if (option.value === 'workspace-write') return t('input.permission.workspaceWrite')
  if (option.value === FULL_ACCESS) return t('input.permission.fullAccess')
...
function submitPermission(command: PermissionSelectProps['command'], id: string, setPick: (value: string | null) => void): void {
  setPick(id)
...
/** A visible second-level permission menu used by the composer's hover menu. */
export function PermissionSubmenu({
  value, locked, command, t, onRequestFullAccess,
}: PermissionSelectProps & { onRequestFullAccess?: () => void }) {
  ...
  const choose = (id: string): void => {
    ...
    submitPermission(command, id, setPick)
  }

  return (
    <div className={css.submenu} role="menu" aria-label={t('input.toolsMode')}>
      {value.options.filter(option => option.value !== 'custom').map((option) => {
        const selected = option.value === (pick ?? value.currentValue)
        const icon = permissionGlyph(option.value)
        return (
          <button
            key={option.value}
            type="button"
            className={clsx(css.submenuItem, selected && css.submenuItemSelected)}
            role="menuitemradio"
            aria-checked={selected}
            disabled={locked || busy}
            onClick={() => { choose(option.value) }}
          >
            {icon !== undefined && <span className={css.submenuIcon} aria-hidden>{icon}</span>}
            <span>{permissionOptionLabel(option, t)}</span>
          </button>
        )
      })}
    </div>
...
export function PermissionSelect({ value, locked, command, t }: PermissionSelectProps) {
  ...
  const items: MenuEntry[] = value.options
    ...
    .map((option) => {
      const icon = permissionGlyph(option.value)
      return { id: option.value, label: permissionOptionLabel(option, t), ...icon === undefined ? {} : { icon } }
    })
      ...
      <Menu
        ...
        anchor={
          <button
            ...
            aria-label={t('input.accessMode', { name: current === undefined ? displayName(currentValue) : permissionOptionLabel(current, t) })}
          ...
          >
            ...
            <span className={css.triggerLabel}>
              {current === undefined ? displayName(currentValue) : permissionOptionLabel(current, t)}
            </span>
