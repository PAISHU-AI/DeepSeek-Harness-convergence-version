...
import type { CallId, ChatStoreState, DetailsPreviewTarget, SelectionTarget } from './contract/views.ts'

/** Declared action shape used to give the exported factory a stable return type. */
type ChatActions = {
  select: (draft: ChatStoreState, target: SelectionTarget | null) => void
  setArtifactPreview: (draft: ChatStoreState, target: DetailsPreviewTarget | null) => void
...
export function createChatStore(): EngineStoreHandle<ChatStoreState, ChatActions> {
  return defineStore({
    ...
    init: (): ChatStoreState => ({ selection: null, artifactPreview: null, draft: '', view: null, inspect: null }),
    persist: 'dsh.conversation.chat',
    actions: {
      select: (d, target: SelectionTarget | null) => { d.selection = target },
      setArtifactPreview: (d, target: DetailsPreviewTarget | null) => { d.artifactPreview = target },
