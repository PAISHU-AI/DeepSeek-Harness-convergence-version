...
interface BenchOptions {
  ...
  addAttachments?: (files: readonly File[]) => Promise<string | null>
...
function bench(over?: BenchOptions) {
  ...
  const props: InputBarProps = {
    ...
    addAttachments: over?.addAttachments,
...
describe('image draft rail', () => {
  it('shows a clearly labelled file-upload control separate from the command menu', () => {
    const { view } = bench()
    expect(view.getByRole('button', { name: '上传文件' })).toBeTruthy()
    expect(view.getByRole('button', { name: '工具' })).toBeTruthy()
    expect(view.queryByRole('button', { name: '命令' })).toBeNull()
    fireEvent.click(view.getByRole('button', { name: '工具' }))
    expect(view.getByText('权限')).toBeTruthy()
    expect(view.getByRole('menuitem', { name: '命令' })).toBeTruthy()
  })

  it('opens the mode submenu on hover instead of embedding a selector in the tools menu', () => {
    const permissions = {
      options: [
    ...
    fireEvent.click(view.getByLabelText('工具'))
    fireEvent.mouseEnter(view.getByRole('menuitem', { name: /权限/ }))
    expect(view.getByRole('menu', { name: '权限' })).toBeTruthy()
    expect(view.getByRole('menuitemradio', { name: '只读' })).toBeTruthy()
    expect(view.getByRole('menuitemradio', { name: '工作区写入' })).toBeTruthy()
    expect(view.getByRole('menuitemradio', { name: '完全访问' })).toBeTruthy()
  })

  it('routes picker documents through typed intake and renders a removable filename chip', async () => {
    const document = {
    ...
    const addAttachments = vi.fn(() => Promise.resolve(null))
    const { view, removeImage } = bench({ attachments: [document], addAttachments })
    expect(view.getByText('brief.txt')).toBeTruthy()
    fireEvent.change(view.getByTestId('composer-file-input'), {
      target: { files: [new File(['new'], 'new.md', { type: 'text/markdown' })] },
    })
    await waitFor(() => expect(addAttachments).toHaveBeenCalledOnce())
    fireEvent.click(view.getByRole('button', { name: '移除文件 brief.txt' }))
    expect(removeImage).toHaveBeenCalledWith('draft-document')
  })
...
describe('running and lock semantics', () => {
  ...
  it('parent-offline running continuable locks Send but keeps independent Stop usable', () => {
    ...
    expect((view.getByLabelText('工具') as HTMLButtonElement).disabled).toBe(true)
    expect(button.getAttribute('aria-label')).toBe('发送消息')
...
describe('command launcher chrome and control seats', () => {
  it('renders the command launcher; the Access chip is absent without the permissions projection; the control seats render EMPTY without entries', () => {
    ...
    expect(view.getByLabelText('工具')).toBeTruthy()
    // Capability absent (no projection value): the chip renders nothing.
    expect(view.queryByLabelText(/^访问模式/)).toBeNull()
    // Every seat dispatched, nothing rendered.
    expect(slotCalls.map(c => c.key)).toEqual(['conversation.input.model'])
    expect(view.queryByLabelText('Plan mode')).toBeNull()
    expect(view.queryByLabelText('Model')).toBeNull()
    fireEvent.click(view.getByLabelText('工具'))
    expect(view.getByLabelText('命令')).toBeTruthy()
    fireEvent.mouseEnter(view.getByRole('menuitem', { name: /^权限/ }))
    expect(slotCalls.map(c => c.key)).toContain('conversation.input.plan')
  })

  it('keeps command choices inside the tools submenu while hovering', () => {
    ...
    expect(view.getByRole('menuitem', { name: /compact/ })).toBeTruthy()
    expect(view.getByRole('button', { name: '工具' }).getAttribute('aria-expanded')).toBe('true')
  })

  it('closes the tools menu when the user presses outside the menu', () => {
    const { view } = bench()
    const trigger = view.getByRole('button', { name: '工具' })
    fireEvent.click(trigger)
    expect(view.getByRole('menu', { name: '工具' })).toBeTruthy()

    fireEvent.pointerDown(document.body)

    expect(view.queryByRole('menu', { name: '工具' })).toBeNull()
  ...
  it('the Access chip renders the projection value and submits a non-Full-access pick directly', async () => {
    ...
    expect((view.getByRole('menuitemradio', { name: '工作区写入' }) as HTMLButtonElement).disabled).toBe(true)
    expect(command).toHaveBeenCalledWith('/permission workspace-write')
    await act(async () => {})
    expect((view.getByRole('menuitemradio', { name: '工作区写入' }) as HTMLButtonElement).disabled).toBe(false)
  })
  ...
  it('requires explicit risk acknowledgement before submitting Full access', async () => {
    ...
    fireEvent.click(view.getByLabelText('工具'))
    fireEvent.mouseEnter(view.getByRole('menuitem', { name: /^权限/ }))
    fireEvent.click(view.getByRole('menuitemradio', { name: '完全访问' }))
    ...
    expect(view.getByRole('menuitemradio', { name: '完全访问' }).getAttribute('aria-checked')).toBe('true')
    await act(async () => {})
  })
  ...
  it('cancels a Full access selection without changing permission and resets acknowledgement', () => {
    ...
    expect(command).not.toHaveBeenCalled()
    expect(view.getByRole('menuitemradio', { name: '工作区写入' }).getAttribute('aria-checked')).toBe('true')
  ...
  it('a registered entry fills its seat and receives the locked owner prop', () => {
    ...
    // A locked composer keeps the secondary menu inaccessible; the persistent
    // model seat remains locked in the same owner contract.
    expect(slotCalls.every(c => (c.owner as { locked: boolean }).locked)).toBe(true)
    cleanup()
    const live = bench({ running: true, planEntry: <i data-testid="live-plan-entry" /> })
    fireEvent.click(live.view.getByLabelText('工具'))
    fireEvent.mouseEnter(live.view.getByRole('menuitem', { name: /^权限/ }))
    expect(live.view.getByTestId('live-plan-entry')).toBeTruthy()
  ...
  it('disabled locks the Access chip and command launcher (running does not)', () => {
    ...
    fireEvent.click(live.view.getByLabelText('工具'))
    fireEvent.mouseEnter(live.view.getByRole('menuitem', { name: /^权限/ }))
    expect((live.view.getByRole('menuitemradio', { name: '工作区写入' }) as HTMLButtonElement).disabled).toBe(false)
  })
})
