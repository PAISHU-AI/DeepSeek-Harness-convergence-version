...
describe('ConversationController', () => {
  ...
  it('serializes locally extracted documents and original images in the selected attachment order', async () => {
    const b = await bench()
    const created = vi.spyOn(URL, 'createObjectURL').mockReturnValue('blob:draft-image')
    try {
      const attachments = await b.root.createDraftAttachments([
        new File(['Document body'], 'brief.txt', { type: 'text/plain' }),
        new File([Uint8Array.of(1, 2, 3)], 'chart.png', { type: 'image/png' }),
      ])
      ...
      expect(b.prompt).toHaveBeenCalledWith([
        ...
        { type: 'image', mediaType: 'image/png', data: 'AQID', name: 'chart.png' },
  ...
  it('refuses a document batch whose extracted text would exceed the message context cap', async () => {
    const b = await bench()
    try {
      const documents = Array.from({ length: 3 }, (_, index) =>
        new File(['x'.repeat(100_001)], `part-${index}.txt`, { type: 'text/plain' }))
      await expect(b.root.createDraftAttachments(documents)).rejects.toMatchObject({ reason: 'batch-text-too-large' })
    } finally {
      await b.runtime.dispose()
    }
  })
