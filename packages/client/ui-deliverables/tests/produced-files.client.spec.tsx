...
describe('produced-file Turn data', () => {
  it('deduplicates paths in first-seen order and stops at the closing Assistant seq', () => {
    ...
    expect(selectProducedFiles(tailOwner(undefined, 9, () => {}, undefined, 2))).toBeNull()
  })
...
describe('ProducedFiles row', () => {
  ...
  it('keeps one measured line, previews a file, and opens the workspace folder', () => {
  ...
  it('keeps the folder action absent without overflow or a local native opener', () => {
    ...
    const previewFile = vi.fn<(path: string) => void>()
    const view = render(
      <ProducedFiles matched={['a.md']} openFile={openFile} previewFile={previewFile} {...capability(true)} t={t} />,
    )
    ...
    for (const unavailable of [capability(false), capability(true, false), capability(undefined)]) {
      view.rerender(<ProducedFiles matched={overflowing} openFile={openFile} previewFile={previewFile} {...unavailable} t={t} />)
      expect(view.queryByRole('button', { name: '在文件夹中显示' })).toBeNull()
    }
  })
