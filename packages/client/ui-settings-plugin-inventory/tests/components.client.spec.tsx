...
function props(
  ...
  setEnabled: PluginInventorySettingsTabInjected['setEnabled'] = async () => { throw new Error('not configured') },
): PluginInventorySettingsTabProps {
  return {
    t,
    list,
...
describe('PluginInventorySettingsTab', () => {
  ...
  it('shows a Chinese purpose on hover and refreshes after a real enablement change', async () => {
    const setEnabled = vi.fn<PluginInventorySettingsTabInjected['setEnabled']>()
      .mockResolvedValue({ ...SNAPSHOT.entries[0]!, enabled: false, fiberPhase: null })
    const list = vi.fn<PluginInventorySettingsTabInjected['list']>()
      .mockResolvedValueOnce(SNAPSHOT)
      .mockResolvedValueOnce({ entries: [{ ...SNAPSHOT.entries[0]!, enabled: false, fiberPhase: null }] })
    render(<PluginInventorySettingsTab {...props(list, setEnabled)} />)

    const toggle = await screen.findByRole('switch', { name: '切换 hmr' })
    expect(toggle.getAttribute('title')).toContain('热更新')
    fireEvent.click(toggle)
    await waitFor(() => { expect(setEnabled).toHaveBeenCalledWith('8a1b2c3d', false) })
    await waitFor(() => { expect(screen.getByText(en.disabledTag)).toBeTruthy() })
  })
