...
/** One preview/edit response must remain small enough for an interactive RPC. */
const MAX_WORKSPACE_ARTIFACT_BYTES = 4 * 1024 * 1024

/** Text formats intentionally editable in the right-side artifact panel. */
const TEXT_ARTIFACT_EXTENSIONS = new Set([
  '.css', '.csv', '.html', '.htm', '.js', '.json', '.jsx', '.md', '.mdx',
  '.mjs', '.py', '.sh', '.svg', '.ts', '.tsx', '.txt', '.xml', '.yaml', '.yml',
])

/** Raster formats returned as a bounded data payload for in-panel display. */
const IMAGE_ARTIFACT_MIME_TYPES = new Map([
  ['.avif', 'image/avif'],
  ['.gif', 'image/gif'],
  ['.jpeg', 'image/jpeg'],
  ['.jpg', 'image/jpeg'],
...
/** Original-format documents the right panel can safely carry as bounded bytes. */
const DOCUMENT_ARTIFACT_MIME_TYPES = new Map([
  ['.pdf', { format: 'pdf' as const, mimeType: 'application/pdf' }],
  ['.docx', { format: 'docx' as const, mimeType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' }],
])

/** Workspace-local files that must never be exposed in an artifact preview. */
const PRIVATE_WORKSPACE_SEGMENTS = new Set(['.git', '.dsh', 'node_modules'])
const PRIVATE_WORKSPACE_BASENAMES = /^(?:\.env(?:\..*)?|credentials(?:\.json|\.ya?ml)?|\.credentials\.ya?ml)$/i
  ...
  for (const item of prepared) {
    ...
    if (item.type === 'document') {
      blocks.push({
        ...
        document: { name: item.name, format: item.format, truncated: item.truncated },
      })
      continue
    }
    {
...
type WorkspaceArtifactErrorCode =
  | 'workspace-file-invalid'
  | 'workspace-file-not-found'
  | 'workspace-file-too-large'
  | 'workspace-file-unsupported'

/** Typed local-file failure, narrowed to a stable API error at the RPC edge. */
class WorkspaceArtifactError extends Error {
  constructor(
    readonly code: WorkspaceArtifactErrorCode,
    readonly path: string,
    readonly maxBytes?: number,
    message?: string,
  ) {
    super(message ?? `workspace artifact ${JSON.stringify(path)} is unavailable`)
    this.name = 'WorkspaceArtifactError'
  }
}

/** Whether a resolved path is the workspace root itself or a proper descendant. */
function isWithinWorkspace(workspacePath: string, candidate: string): boolean {
  const remainder = relative(workspacePath, candidate)
  return remainder === '' || (!remainder.startsWith('../') && remainder !== '..' && !isAbsolute(remainder))
}

/** Validate the wire-relative artifact name before it reaches Node path resolution. */
function validateArtifactRelativePath(path: string): void {
  const segments = path.split('/')
  if (
    path === ''
    || isAbsolute(path)
    || path.includes('\\')
    || segments.some(segment => segment === '' || segment === '.' || segment === '..' || PRIVATE_WORKSPACE_SEGMENTS.has(segment))
    || PRIVATE_WORKSPACE_BASENAMES.test(segments.at(-1) ?? '')
  ) {
    throw new WorkspaceArtifactError('workspace-file-invalid', path, undefined, 'artifact path must be a safe workspace-relative file path')
  }
}

/** Resolve an artifact path and prove its existing parent remains in the registered workspace. */
async function resolveWorkspaceArtifactPath(workspacePath: string, path: string): Promise<{ root: string; candidate: string }> {
  validateArtifactRelativePath(path)
  const root = await realpath(workspacePath)
  const candidate = resolve(root, path)
  if (!isWithinWorkspace(root, candidate)) {
    throw new WorkspaceArtifactError('workspace-file-invalid', path, undefined, 'artifact path escapes its workspace')
  }
  try {
    const parent = await realpath(dirname(candidate))
    if (!isWithinWorkspace(root, parent)) {
      throw new WorkspaceArtifactError('workspace-file-invalid', path, undefined, 'artifact parent escapes its workspace')
    }
  } catch (error: unknown) {
    if (error instanceof WorkspaceArtifactError) throw error
    throw new WorkspaceArtifactError('workspace-file-not-found', path, undefined, 'artifact parent does not exist')
  }
  return { root, candidate }
}

/** Read a bounded, known-safe text or raster artifact from one registered workspace. */
async function readWorkspaceArtifact(workspacePath: string, path: string): Promise<
  | { kind: 'text'; content: string }
  | { kind: 'image'; mimeType: string; data: string }
  | { kind: 'binary'; format: 'pdf' | 'docx'; mimeType: string; data: string }
> {
  const { root, candidate } = await resolveWorkspaceArtifactPath(workspacePath, path)
  let target: string
  try {
    target = await realpath(candidate)
  } catch {
    throw new WorkspaceArtifactError('workspace-file-not-found', path, undefined, 'artifact file does not exist')
  }
  if (!isWithinWorkspace(root, target)) {
    throw new WorkspaceArtifactError('workspace-file-invalid', path, undefined, 'artifact file escapes its workspace')
  }
  const info = await stat(target)
  if (!info.isFile()) {
    throw new WorkspaceArtifactError('workspace-file-invalid', path, undefined, 'artifact path is not a regular file')
  }
  if (info.size > MAX_WORKSPACE_ARTIFACT_BYTES) {
    throw new WorkspaceArtifactError('workspace-file-too-large', path, MAX_WORKSPACE_ARTIFACT_BYTES)
  }
  const extension = extname(path).toLowerCase()
  const imageMimeType = IMAGE_ARTIFACT_MIME_TYPES.get(extension)
  if (imageMimeType !== undefined) {
    return { kind: 'image', mimeType: imageMimeType, data: (await readFile(target)).toString('base64') }
  }
  const document = DOCUMENT_ARTIFACT_MIME_TYPES.get(extension)
  if (document !== undefined) {
    return { kind: 'binary', ...document, data: (await readFile(target)).toString('base64') }
  }
  if (!TEXT_ARTIFACT_EXTENSIONS.has(extension)) {
    throw new WorkspaceArtifactError('workspace-file-unsupported', path, undefined, 'artifact format is not previewable')
  }
  const bytes = await readFile(target)
  if (bytes.includes(0)) {
    throw new WorkspaceArtifactError('workspace-file-unsupported', path, undefined, 'artifact is not UTF-8 text')
  }
  return { kind: 'text', content: bytes.toString('utf8') }
}

/** Replace a bounded text artifact without following paths outside its workspace. */
async function writeWorkspaceArtifact(workspacePath: string, path: string, content: string): Promise<number> {
  if (!TEXT_ARTIFACT_EXTENSIONS.has(extname(path).toLowerCase())) {
    throw new WorkspaceArtifactError('workspace-file-unsupported', path, undefined, 'artifact format is not editable text')
  }
  ...
  if (!linkInfo.isFile() || linkInfo.isSymbolicLink()) {
    throw new WorkspaceArtifactError('workspace-file-invalid', path, undefined, 'artifact path is not a writable regular file')
  }
...
/** Fold a local artifact failure into the wire's closed business-error union. */
function workspaceArtifactFailure<T>(
  request: RpcRequest<{ workspaceId: WorkspaceId; path: string }>,
  error: WorkspaceArtifactError,
): RpcResponse<T> {
  const details = { workspaceId: request.payload.workspaceId, path: error.path }
  switch (error.code) {
    case 'workspace-file-too-large': {
      const maxBytes = error.maxBytes ?? MAX_WORKSPACE_ARTIFACT_BYTES
      return err(request, {
        code: error.code,
        message: error.message,
        details: { ...details, maxBytes },
      })
    }
    default:
...
/** Decode a canvas result without accepting malformed or oversized base64. */
function decodeWorkspaceImage(path: string, data: string): Buffer {
  const bytes = Buffer.from(data, 'base64')
  if (data.length === 0 || bytes.toString('base64') !== data) {
    throw new WorkspaceArtifactError('workspace-file-invalid', path, undefined, 'image data must be canonical base64')
  }
  if (bytes.byteLength > MAX_WORKSPACE_ARTIFACT_BYTES) {
    throw new WorkspaceArtifactError('workspace-file-too-large', path, MAX_WORKSPACE_ARTIFACT_BYTES)
  }
  return bytes
}

/** Write a rendered canvas result to the same known image file. */
async function writeWorkspaceImage(workspacePath: string, path: string, data: string): Promise<number> {
  if (!IMAGE_ARTIFACT_MIME_TYPES.has(extname(path).toLowerCase())) {
    throw new WorkspaceArtifactError('workspace-file-unsupported', path, undefined, 'artifact format is not an editable image')
  }
  const bytes = decodeWorkspaceImage(path, data)
  const { root, candidate } = await resolveWorkspaceArtifactPath(workspacePath, path)
  let linkInfo
  try {
    linkInfo = await lstat(candidate)
  ...
  return {
    ...
    workspace: {
      ...
      async readFile(request) {
        ...
        try {
          return ok(request, await readWorkspaceArtifact(workspace.path, path))
        } catch (error: unknown) {
          if (error instanceof WorkspaceArtifactError) return workspaceArtifactFailure(request, error)
          throw error
        }
      },

      async openFile(request, signal) {
        const { workspaceId, path } = request.payload
        const workspace = ctx.workspaceRegistry.get(brandWorkspaceId(workspaceId))
        if (workspace === undefined) return workspaceNotFound(request, workspaceId)
        try {
          const { root, candidate } = await resolveWorkspaceArtifactPath(workspace.path, path)
          const target = await realpath(candidate)
          if (!isWithinWorkspace(root, target)) {
            throw new WorkspaceArtifactError('workspace-file-invalid', path, undefined, 'artifact file escapes its workspace')
          }
          ...
          return await openPath(request, target, signal)
        } catch (error: unknown) {
          if (error instanceof WorkspaceArtifactError) return workspaceArtifactFailure(request, error)
          if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
            return workspaceArtifactFailure(request, new WorkspaceArtifactError('workspace-file-not-found', path))
          }
          throw error
        }
      },
      ...
      async writeFile(request) {
        ...
        if (workspace === undefined) return workspaceNotFound(request, workspaceId)
        try {
          const bytes = 'content' in request.payload
            ? await writeWorkspaceArtifact(workspace.path, path, request.payload.content)
            : await writeWorkspaceImage(workspace.path, path, request.payload.data)
