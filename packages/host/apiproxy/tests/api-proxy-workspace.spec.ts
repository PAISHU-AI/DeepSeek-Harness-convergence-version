...
describe('workspace artifact files', () => {
  it('reads a text artifact inside its registered workspace and rejects path escape', async () => {
    const { api, root } = await harness()
    const workspace = expectOk(await api.workspace.create(request({ path: root }))).workspace
    writeFileSync(join(root, 'report.md'), '# Preview\n')

    const files = api.workspace as unknown as {
      readFile(request: RpcRequest<{ workspaceId: WorkspaceId; path: string }>): Promise<RpcResponse<{
        kind: 'text'
        content: string
      }>>
    }

    expect(files.readFile).toBeTypeOf('function')
    expect((await files.readFile(request({ workspaceId: workspace.workspaceId, path: 'report.md' }))).result)
      .toEqual({ ok: true, value: { kind: 'text', content: '# Preview\n' } })
    expect((await files.readFile(request({ workspaceId: workspace.workspaceId, path: '../outside.md' }))).result)
      .toMatchObject({ ok: false, error: { code: 'workspace-file-invalid' } })
  })

  it('returns PDF and Word artifacts as bounded original binary payloads', async () => {
    const { api, root } = await harness()
    const workspace = expectOk(await api.workspace.create(request({ path: root }))).workspace
    writeFileSync(join(root, 'report.pdf'), Buffer.from('%PDF-1.7'))
    writeFileSync(join(root, 'brief.docx'), Buffer.from('PK\\x03\\x04'))

    const pdf = await api.workspace.readFile(request({ workspaceId: workspace.workspaceId, path: 'report.pdf' }))
    expect(pdf.result).toEqual({ ok: true, value: { kind: 'binary', format: 'pdf', mimeType: 'application/pdf', data: Buffer.from('%PDF-1.7').toString('base64') } })
    const docx = await api.workspace.readFile(request({ workspaceId: workspace.workspaceId, path: 'brief.docx' }))
    expect(docx.result).toEqual({ ok: true, value: { kind: 'binary', format: 'docx', mimeType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', data: Buffer.from('PK\\x03\\x04').toString('base64') } })
  })

  it('opens an original document through its resolved workspace path only', async () => {
    const opened: string[] = []
    ...
    writeFileSync(join(root, 'brief.docx'), Buffer.from('PK\\x03\\x04'))
    const files = api.workspace as unknown as {
      openFile(request: RpcRequest<{ workspaceId: WorkspaceId; path: string }>, signal: AbortSignal): Promise<RpcResponse<{ opened: true }>>
    }

    expect((await files.openFile(request({ workspaceId: workspace.workspaceId, path: 'brief.docx' }), new AbortController().signal)).result)
      .toEqual({ ok: true, value: { opened: true } })
    expect(opened).toEqual([join(root, 'brief.docx')])
    expect((await files.openFile(request({ workspaceId: workspace.workspaceId, path: '../outside.docx' }), new AbortController().signal)).result)
      .toMatchObject({ ok: false, error: { code: 'workspace-file-invalid' } })
  })

  it('replaces text and image artifacts while rejecting a mismatched write mode', async () => {
    const { api, root } = await harness()
    const workspace = expectOk(await api.workspace.create(request({ path: root }))).workspace
    const path = join(root, 'report.md')
    writeFileSync(path, '# Before\n')
    const files = api.workspace as unknown as {
      writeFile(request: RpcRequest<{ workspaceId: WorkspaceId; path: string; content: string }>): Promise<RpcResponse<{ bytes: number }>>
    }

    expect((await files.writeFile(request({
      workspaceId: workspace.workspaceId,
      path: 'report.md',
      content: '# After\n',
    }))).result).toEqual({ ok: true, value: { bytes: 8 } })
    expect(readFileSync(path, 'utf8')).toBe('# After\n')
    expect((await files.writeFile(request({
      workspaceId: workspace.workspaceId,
      path: 'image.png',
      content: 'not an image',
    }))).result).toMatchObject({ ok: false, error: { code: 'workspace-file-unsupported' } })
  })

  it('writes a canonical raster payload back to its original image file', async () => {
    const { api, root } = await harness()
    const workspace = expectOk(await api.workspace.create(request({ path: root }))).workspace
    const image = Buffer.from([0x89, 0x50, 0x4e, 0x47])
    writeFileSync(join(root, 'canvas.png'), Buffer.from('old'))
    const files = api.workspace as unknown as {
      writeFile(request: RpcRequest<{ workspaceId: WorkspaceId; path: string; data: string }>): Promise<RpcResponse<{ bytes: number }>>
    }
    ...
    }))).result).toEqual({ ok: true, value: { bytes: 4 } })
    expect(readFileSync(join(root, 'canvas.png'))).toEqual(image)
  })
})
