  ...
  return {
    ...
    workspace: {
      ...
      async readFile(request) {
        return { rpcId: request.rpcId, result: { ok: true, value: { kind: 'text' as const, content: `fixture:${request.payload.path}` } } }
      },
      async openFile(request) {
        return { rpcId: request.rpcId, result: { ok: true, value: { opened: true as const } } }
      },
      async writeFile(request) {
        return { rpcId: request.rpcId, result: { ok: true, value: { bytes: 0 } } }
      },
    },
    agentPresets: {
