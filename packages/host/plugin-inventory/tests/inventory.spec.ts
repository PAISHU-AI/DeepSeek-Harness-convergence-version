...
describe('PluginInventoryGateway', () => {
  ...
  it('transactionally enables and disables an existing Loader entry', async () => {
    const { ctx, inventory } = await harness()
    const entryId = await ctx.loader.create({ name: 'cordis:active' }) as PluginEntryId

    await expect(inventory.setEnabled(entryId, false)).resolves.toMatchObject({
      entryId,
      enabled: false,
      fiberPhase: null,
    })
    await expect(inventory.setEnabled(entryId, true)).resolves.toMatchObject({
      entryId,
      enabled: true,
      fiberPhase: 'active',
    })
